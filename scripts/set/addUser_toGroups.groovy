...
def userToAdd = """
c.finizio;Cliente,Tester;Digital@
r.silvestro;Cliente,Tester;AFC@
a.brunetti;Cliente,Tester;CC&LO@
r.calcagno;Cliente,Demand,Tester;Betting,Digital@
m.lettieri;Cliente,Tester;CC&LO@
m.franco;Cliente,Tester;CC&LO@
a.vaccaro;Cliente,Tester;Betting@
d.salomone;Cliente,Tester;AFC@
c.salina;Cliente,Tester;Digital@
m.ricciotti;Cliente,Tester;AWP,VLT@
"""
...
for (int i = 0; i < users_info.length - 1; i++){
    ...
    if(info.length != 3){
        log.warn("${username} non è stato aggiornato perché ruolo o progetto sono vuoti")
    }else{
...
void addUsersToGroups (String user, ArrayList <String> groupsName) {
    ...
    if(currUser == null){
        log.warn("errore, non esiste il seguente user ${user}")
    }else{
      if (currUser.active) {
            groupsName.each{
